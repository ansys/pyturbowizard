...
def Fplot(solver, file_name, work_dir, case_dict=None):
    ...
    def get_loading_data(surf,span=None):
        ...
        fields = ["x-coordinate","pressure","total-pressure","rel-total-pressure","mach-number","rel-mach-number",
    ...
    def plot_loading(data, fields, SaveFig=True, w_dir=None):
        ...
        bx = (axial_coord - np.min(axial_coord)) / (np.max(axial_coord) - np.min(axial_coord))  # Axial Chord
        ...
        for field in fields:
            if field == "psopt":
                ...
                yvar = data['pressure'] / max(data['rel-total-pressure'])  # Ps/Max(Pt)_along surface
                ylabel = "Ps/Pt"
        ...
        for af in list(prof.keys()):
            for zone in list(prof[af].keys()):
                ...
                for vv in list(prof[af][zone].keys())[1:]:                
                    ...
                    title = clean_af.title() + " " + zone.title() + " " + vv.title().replace("-"," ") + " Radial Profile"
                    ax.set_title(title)
                    ax.set_xlim(0,1.0)
                    ax.grid(visible=True)
                    ax.set_xlabel('% Span')
    ...
    def generate_radial_profiles(solver, work_dir, config):
        ...
        for row_name, settings in rows.items():
            ...
            for var in variables:
                fname_inlet = os.path.join(profile_dir, f"{row_name}_inlet_{var.replace('-','_')}_radial_profile.xy")
                fname_outlet = os.path.join(profile_dir, f"{row_name}_outlet_{var.replace('-','_')}_radial_profile.xy")

                solver.tui.turbo_post.xy_plot_avg(topo, 'hub-to-casing-distance', var, str(inlet_axial), '0', '0', "yes", f"\"{fname_inlet}\"")
                solver.tui.turbo_post.xy_plot_avg(topo, 'hub-to-casing-distance', var, str(outlet_axial), '0', '0', "yes", f"\"{fname_outlet}\"")

                prof_data[row_name]["Inlet"].update(Read_Fluent_xy(fname_inlet))
                prof_data[row_name]["Outlet"].update(Read_Fluent_xy(fname_outlet))

        # Save plots
        plot_row_radial_profile(prof_data, SaveFig=True, fig_dir=os.path.join(work_dir, "Radial_Profile_Figures"))
