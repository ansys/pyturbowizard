...
def studyPlot(data):
    ...
    for studyName in data:
        ...
        if os.path.isfile(design_point_table_path):
            ...
            MP_MassFlow_Out = pd.to_numeric(
                design_point_table.loc[:, "MP_OUT_MassFlow"], errors="coerce"
            )
            MP_PR_tot = pd.to_numeric(
            ...
            axs[0].set_xlim(
                [MP_MassFlow_Out.min() * 0.99, MP_MassFlow_Out.max() * 1.01]
            )
